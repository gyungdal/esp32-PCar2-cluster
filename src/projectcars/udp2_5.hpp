/*************************************************************************************************
https://www.projectcarsgame.com/project-cars-2-api/
  Description:
    Base definitions of udp packet structure
    The data definition mostly follows the data set for the Shared memory, so it
is strongly suggested to have a look to the latest shared memory header if you
have problem decoding any data.
...
enum EUDPStreamerPacketHandlerType {
  ...
  eTimings = 3,
  eGameState = 4,
  eWeatherState = 5,  // not sent at the moment, information can be found in the
                      // game state packet
...
/*
Each packet holds a base header with identification info to help with UDP
unreliability.
*/

struct PacketBase {
  unsigned int mPacketNumber;  // 0 counter reflecting all the packets that have
                               // been sent during the game run
  unsigned int mCategoryPacketNumber;  // 4 counter of the packet groups
                                       // belonging to the given category
  unsigned char
      mPartialPacketIndex;  // 8 If the data from this class had to be sent in
                            // several packets, the index number
  unsigned char
      mPartialPacketNumber;  // 9 If the data from this class had to be sent in
                             // several packets, the total number
  unsigned char mPacketType;  // 10 what is the type of this packet (see
                              // EUDPStreamerPacketHanlderType for details)
  unsigned char
      mPacketVersion;  // 11 what is the version of protocol for this handler,
                       // to be bumped with data structure change
};  // 12 total size
...
//	When it is sent: Counter resets on entering InRace state and again each
time any of the values changes
//
*******************************************************************************************************************/
...
struct sRaceData {
  ...
  unsigned short
      sLapsTimeInEvent;  // 304 contains lap number for lap based session or
                         // quantized session duration (number of 5mins) for
                         // timed sessions, the top bit is 1 for timed sessions
...
//	The sParticipantsChangedTimestamp represent last time the participants
has changed and is to be used to sync
...
struct sParticipantsData {
  ...
  char sName[PARTICIPANTS_PER_PACKET]
            [PARTICIPANT_NAME_LENGTH_MAX];             //	16	1024
  unsigned int sNationality[PARTICIPANTS_PER_PACKET];  //	1040 64
  unsigned short sIndex[PARTICIPANTS_PER_PACKET];  //  1104 32 -- session unique
                                                   //  index for MP races
};                                                 //	1136
...
struct sParticipantInfo {
  ...
  signed short
      sOrientation[3];  // 6 -- Quantized heading (-PI .. +PI) , Quantized pitch
                        // (-PI / 2 .. +PI / 2),  Quantized bank (-PI .. +PI).
  unsigned short sCurrentLapDistance;  // 12 --
  unsigned char sRacePosition;  // 14 -- holds the race position, + top bit
                                // shows if the participant is active or not
  unsigned char
      sSector;  // 15 -- sector + extra precision bits for x/z position
  unsigned char
      sHighestFlag;  // 16 -- (enum 3 bits/enum 2 bits) Flag colour and reason
  unsigned char sPitModeSchedule;  // 17 -- (enum 3 bits/enum 2 bits) Pit mode
                                   // and Pit schedule
  unsigned short sCarIndex;  // 18 -- top bit shows if participant is (local or
                             // remote) human player or not
  unsigned char
      sRaceState;  // 20 -- race state flags + invalidated lap indication --
  unsigned char sCurrentLap;  // 21 --
  float sCurrentTime;         // 22 --
  float sCurrentSectorTime;   // 26 --
  unsigned short
      sMPParticipantIndex;  // 30 --  matching sIndex from sParticipantsData
...
struct sTimingsData {
  ...
  float sEventTimeRemaining;  // 17  // time remaining, -1 for invalid time,  -1
                              // - laps remaining in lap based races  --
  float sSplitTimeAhead;      // 21 --
  float sSplitTimeBehind;     // 25 --
  float sSplitTime;           // 29 --
  sParticipantInfo
      sPartcipants[UDP_STREAMER_PARTICIPANTS_SUPPORTED];  // 33 1024
  unsigned short sLocalParticipantIndex;  // 1057 -- identifies which of the MP
                                          // participants is the local player
...
//	Game State.
//
//	Frequency: Each 5s while being in Main Menu, Each 10s while being in
race + on each change Main Menu<->Race several times.
//	the frequency in Race is increased in case of weather timer being faster
up to each 5s for 30x time progression
...
struct sGameStateData {
  ...
  char mGameState;  // 15 -- first 3 bits are used for game state enum, second 3
                    // bits for session state enum See shared memory example file
                    // for the enums
  signed char sAmbientTemperature;  // 16
  signed char sTrackTemperature;    // 17
  unsigned char sRainDensity;       // 18
  unsigned char sSnowDensity;       // 19 --  this will be non zero only in Snow
                               // season, in other seasons whatever is falling
                               // from the sky is reported as rain
...
//
//	Frequency: When entering the race and each time any of the values
change, so basically each time any of the participants
...
struct sParticipantStatsInfo {
  ...
  unsigned int
      sParticipantOnlineRep;  // 24 (unsigned short rank type + unsigned short
                              // strength, 0 in SP races)
  unsigned short
      sMPParticipantIndex;  // 28 -- matching sIndex from sParticipantsData
};                          // 32 -- padded to  32

struct sParticipantsStats {
  sParticipantStatsInfo
      sParticipants[UDP_STREAMER_PARTICIPANTS_SUPPORTED];  // 1024
};
...
//	this information with the rest of the participant related packets
//
//	Note: This data is always sent with at least 2 packets. The 1-(n-1)
holds the vehicle name for each participant
